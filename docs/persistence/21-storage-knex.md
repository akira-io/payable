...
transaction. Each catalog mutation writes either its product or price row together with one audit
record and one outbox event. A failure inside the transaction rolls back all three local writes. The
provider call occurs outside this SQL transaction and completes before the local writes begin.
